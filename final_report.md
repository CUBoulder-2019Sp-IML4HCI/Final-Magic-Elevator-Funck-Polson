...
Magicians wield magic wands that are plastic toy Harry Potter wands whose insides were gutted and replaced with a Micro:bit that senses X, Y, and Z acceleration. They perform wand motions in tandem with voice incantations that correspond to calling the elevator either up or down. They come up with and train the motions and incantations themselves via the Magic Elevator user interface (UI). When spells are correctly cast, a button-pushing mechanism built almost entirely out of LEGO parts that is mounted over the elevator buttons summons the elevator in the appropriate direction.
